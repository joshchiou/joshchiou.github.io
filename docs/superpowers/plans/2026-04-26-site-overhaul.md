...
- [ ] **Replace parse_takeout.py with parse_location_history.py (new format)**

Google deprecated the Semantic Location History (Takeout) format. The new export is a single `location-history.json` downloaded directly from Google Maps → Timeline → Export timeline data (JSON). The new format is a flat array with `geo:lat,lon` coordinates only (no address strings), requiring Nominatim reverse geocoding.

```bash
git rm scripts/parse_takeout.py
```
...
- Skips Home/Work semantic types automatically
- Applies `MIN_VISIT_HOURS = 4` and `MIN_CONFIDENCE = 0.5` filters after geocoding

Add the cache file to `.gitignore` so it isn't committed:

```bash
echo "scripts/.geocode_cache.json" >> .gitignore
```
...
Loading: /path/to/location-history.json
  ...
  Geocoding 289 new places at ~1/sec (~289s, cache already has 0 entries)...
  20/289 geocoded
  40/289 geocoded
  ...
  289/289 geocoded
  Cache saved to scripts/.geocode_cache.json (289 total entries)

Building output...
  Processed N qualifying visits
  Skipped: 97 sub-visits, 306 Home/Work, ...

Results: N countries, N unique city-level places

  → _data/travel_countries.yml  (N entries)
  → _data/travel_cities.yml  (N entries)
```

Review `_data/travel_cities.yml` and remove entries that are not real destinations (restaurants, shops, etc.) before committing.
...
git add scripts/update_strava.py .github/workflows/update-strava.yml \
        ...
        scripts/parse_location_history.py .gitignore
git commit -m "feat: add Strava pipeline and location history parser for travel map"
```
...
- **Travel data:** Run `python scripts/parse_location_history.py /path/to/location-history.json` (already downloaded to `/opt/docker/openclaw/data/.openclaw/workspace/tmp/google_maps/location-history.json`); review and commit `_data/travel_countries.yml` and `_data/travel_cities.yml`.
